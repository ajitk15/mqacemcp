...
| ACE integration-server / EG run-state, **start time / restart time / uptime**, PID — "when did EG X restart", "when was EG X last started", "how long has EG Y been up" | `ace_node_overview` | `nodes` OPTIONAL (a LIST; omit to cover every configured node). Read each EG's **`startup_time`** (also `active.startupTime`; PID = `active.processId`) — that is the start of the currently running process, i.e. the LAST restart. Uptime = now − `startup_time`. **NEVER answer this from `ace_search`** — the offline dump has no event times. There is NO restart history: if the user wants previous restarts or a restart count, give the last start time and say the history is not exposed by any tool (it lives in the ACE syslog / event log). |
...
| "Find any ACE thing matching X" / "BIP errors mentioning X" / "list nodes" | `ace_search` | `search_strings` required (a LIST — one or more substrings; `[""]` = list all); `scope` optional (`nodes`/`dump`/`all`); `server` / `application` / `node` optional and matched EXACTLY. **`server` takes an EG name and `node` takes a NODE name — for "... on NODE1" pass `node="NODE1"`, NEVER `server="NODE1"`.** A term that names a known EG is auto-scoped to that EG (`match_kind: exact-eg`) and other loose terms in the same call are dropped, so do NOT pad the list with words like "Application" or "running" — they only appear in `ignored_search_strings`. |
...
- User: "everything in the ACE dump mentioning ACE_MQ_group_messages on NODE1"
    → `ace_search(search_strings=["ACE_MQ_group_messages"], node="NODE1")`   // the node goes in `node`, NOT in `server` (which takes EG names only)
...
NOTE: ACE RESTART **HISTORY** is not covered — how many times an EG has restarted, when it restarted before the current run, or any restart timeline. Only the CURRENT process start is available (`ace_node_overview` → `startup_time`). Give that last start time first, then escalate for the history, saying it lives in the ACE syslog / event log. Do NOT substitute the extract time and do NOT invent a history. The equivalent MQ limit applies to `DISPLAY QMSTATUS`, which likewise reports only the current start (`STARTDA`/`STARTTI`).

...
|---|---|---|---|---|
| 1 | LLM reasoner | OpenAI GPT-5.5 via `langchain-openai` | `chatbot/backend/agent.py` (`ChatOpenAI(model=…)`) | ✅ |
...
| 6 | Session memory | LangGraph `MemorySaver` checkpointer, keyed by `thread_id` from the frontend's `st.session_state` | `chatbot/backend/agent.py` (`checkpointer=MemorySaver()`); `chatbot/frontend/app.py` (per-session `thread_id`) | ✅ in-process (deliberately; v1) |
| 7 | Output formatter | Two layers: (a) backend `renderers.py` infers tables / mermaid / code / text from tool output shape; (b) the LLM is prompted to emit Mermaid for relationships and rely on auto-tables for lists | `chatbot/backend/renderers.py`; prompt in `chatbot/backend/prompts/system.md` | ✅ |
| 8 | Guardrails | (a) **Scope refusal** via `BOT_DOMAIN`; (b) **Tool allow/deny list**; (c) **Read-only enforcement** in the MCP server itself (blocks `ALTER`/`DEFINE`/etc.); (d) **Hostname allow-list** so even an exploited agent can't reach prod hosts; (e) **Error sanitisation** so raw tracebacks never reach the user | `chatbot/backend/agent.py` (SCOPE_BLOCK_TEMPLATE), `chatbot/backend/mcp_client.py` (`_filter_tools`), `server/safety.py`, `server/errors.py` | ✅ multi-layer |
| 9 | Observability | (a) **Per-call JSONL log** on the MCP server side (`logs/queries-*.jsonl`) with tool, args, endpoints, latency, outcome; (b) **Streaming tool-step events** (`tool_call`, `tool_result`) so the UI shows each step the agent took; (c) **`/api/health`** surfaces resolved prompt source, scope, allow/deny lists | `server/query_log.py`, `chatbot/backend/app.py` (event stream + `/api/health`), `chatbot/frontend/renderers.py` (tool-step expander) | ✅ |
| 10 | Streaming I/O | SSE `text/event-stream` of typed events (`token`, `tool_call`, `tool_result`, `final`, `done`); frontend reader assembles incrementally | `chatbot/backend/app.py` (`_run`, `_sse`), `chatbot/frontend/client.py` (SSE reader) | ✅ |
...
`chatbot/backend/agent.py:build_agent`:

...
from server.access_helpers import (
    evaluate_ace_access,
    evaluate_mq_access,
...
def register(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    @logged_tool
    async def user_access_verify(
        user_id: str,
        qmgr_names: list[str] | None = None,
        ace_nodes: list[str] | None = None,
        channel: str | None = None,
        resource: str | None = None,
        detail: str | None = None,
    ) -> str:
        """IBM MQ + IBM ACE: Verify a domain user's configured effective access.

        Resolves the supplied user to direct and nested domain groups in real
        time through read-only LDAPS, then evaluates those groups against the
        offline authorization snapshots. MQ evidence is read from AUTHREC,
        QMGR, and CHLAUTH rows already present in `qmgr_dump.csv`. ACE evidence
        is read from the normalized `ace_auth_dump.csv` generated by fleet
        automation.

        Results use four evidence-safe verdicts: ALLOWED, DENIED, CONDITIONAL,
        and UNKNOWN. MQ returns CONDITIONAL when OAM CONNECT is present but
        CHLAUTH still requires runtime channel evaluation. Missing or stale
        snapshots and unavailable directory lookups never produce a guessed
        allow/deny verdict.

        Pass one or more MQ queue managers, ACE nodes, or both in the same call.
        For example: `user_id="ajit001", qmgr_names=["QM2"]`, or
        `user_id="user111", ace_nodes=["NODE1"], detail="full"`.

        Args:
            user_id: Domain user ID, UPN, or DOMAIN\\user value.
            qmgr_names: Optional queue managers to evaluate.
            ace_nodes: Optional ACE integration nodes to evaluate.
            channel: Optional MQ channel context. CHLAUTH remains conditional
                until a runtime RUNCHECK or equivalent collector evidence exists.
            resource: Optional MQ object profile or ACE resource to scope access.
            detail: `summary` (default) or `full` evidence output.
        """
        user = (user_id or "").strip()
        qms = _as_str_list(qmgr_names)
        nodes = _as_str_list(ace_nodes)
        output_detail = (detail or "summary").strip().lower()
        if not user:
            return json.dumps(
                {"status": "error", "message": "No user ID supplied."}, indent=2
            )
        if not qms and not nodes:
            return json.dumps(
                {
                    "status": "error",
                    "message": (
                        "Supply qmgr_names=[...] and/or ace_nodes=[...] to define "
                        "the access target."
                    ),
                },
                indent=2,
            )
        if output_detail not in {"summary", "full"}:
            return json.dumps(
                {
                    "status": "error",
        ...
        identity = await asyncio.to_thread(resolve_domain_groups, user)
        identity_resolved = identity.get("status") == "success"
        groups = list(identity.get("groups") or [])
        canonical = identity.get("canonical_user")

        mq_results = evaluate_mq_access(
            user,
            groups,
            qms,
        ...
        ace_results = evaluate_ace_access(
            user,
            groups,
            nodes,
            canonical_user=canonical,
        ...
        verdicts = [
            ...
            for result in [*mq_results, *ace_results]
        ]
        ...
        if output_detail == "summary" and identity_resolved:
            relevant = {
                entity.split(":", 1)[1].casefold()
                for result in mq_results
                for entity in result.get("matched_entities", [])
                if ":" in entity
            }
            relevant.update(
                role.get("subject", "").casefold()
                for result in ace_results
                for role in result.get("matched_roles", [])
            )
            safe_identity["groups"] = [
                group for group in groups if group.casefold() in relevant
            ]
            safe_identity["group_count"] = len(groups)
        ...
        response = {
            ...
            "connection_context": {
                "channel": channel,
                "note": (
                    "This tool validates configured authorization evidence; it "
                    "does not authenticate as the end user or open a live connection."
                ),
            },
        }
